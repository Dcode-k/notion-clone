...
const Item = ({id,label,onClick,icon:Icon,active,documentIcon,isSearch,level,onExpand,expanded}:ItemProps) => {
   ...
   const onArchive=(event:React.MouseEvent<HTMLDivElement,MouseEvent>)=>{
    ...
    <div onClick={onClick} role='button' style={{paddingLeft:level ? `${(level * 12)+12}px`:"12px"}} className={cn('group min-h-[27px] text-sm py-1 pr-3 w-full hover:bg-primary/5 flex items-center text-muted-foreground font-medium',active && "bg-primary/5 text-primary")}>
        ...
        {
            isSearch && (
                <kbd className='ml-auto pointer-events-none inline-flex h-5 select-none items-center gap-1 rounded border bg-muted px-1.5 font-medium text-[10px] font-mono text-muted-foreground opacity-100'>
